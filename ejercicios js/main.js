...
function cal () {
    ...
    let tot_com_p=tot_com_t=tot_aloj_p=tot_aloj_t=des_tiq_t=val_tiq_t_n=val_tiq_t=val_t_n=val_t_n_p=val_t_p=val_t=0;
    let res = num_per+6;
    console.log(res);
    ...
    let inx = ciudad==viajes[0][0] ? 0:ciudad==viajes[1][0]?1:ciudad==viajes[2][0]?2:3 ;

    tot_com_p = viajes[inx][1] * viajes[inx][4];
    tot_com_t = tot_com_p*num_per;
    tot_aloj_p = viajes[inx][1] * viajes[inx][2];
    tot_aloj_t =tot_aloj_p*num_per;
    val_tiq_p = viajes[inx][3];
    val_tiq_t= viajes[inx][1] * viajes[inx][3];
    des_tiq_t= ciudad==viajes[0][0] || ciudad==viajes[1][0];
